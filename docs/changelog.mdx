...
- **`GET /api/trade/v1/get-tariff-trends` request validation and coverage reporting** (#6316). Same defect class as the trade-flows fix (#6309), on the premium MFN tariff path. **(1)** `reporting_country` and `partner_country` must now be 3-digit UN M49 codes, `product_sector` is limited to optional alphanumeric/`_`/`-` tokens up to 16 characters, and `years` must be 0–30; a malformed value returns HTTP 400 instead of being silently replaced with `840` / `10`, which previously answered about a country the caller never asked for. Omitted parameters still take the documented defaults (`840`, empty partner, empty sector → All products, `years=10`). **(2)** `years` is no longer restricted in practice to 10. The seed now holds a 30-year window per reporter under `trade:tariffs:v2:{reporter}` and the endpoint slices it, so every lookback in 1–30 is served; requests for any other window previously returned `upstream_unavailable: true`. **(3)** The response gains `unavailable_reason` — the `TariffTrendUnavailableReason` enum (`NOT_COVERED`, `SEED_MISSING`, `COVERAGE_UNKNOWN`, `CACHE_UNAVAILABLE`, `INVALID_REQUEST`, `UNSPECIFIED` when datapoints are served) — plus `coverage_start_year` / `coverage_end_year`. `upstream_unavailable` is now `false` for `NOT_COVERED`. **Coverage truth:** WTO indicator `TP_A_0010` is an MFN applied average for the reporting economy — it has no partner dimension and no product-sector split beyond the All-products aggregate. `partner_country` is therefore accepted for forward compatibility and never changes the answer (callers that pass a partner, including the dashboard's own `fetchTariffTrends('840','156','',10)`, keep working). `product_sector` empty or `all` is the only covered sector; any other value is answered `NOT_COVERED`. Fleet health is the `tariffTrendsUs` series, now probing `seed-meta:trade:tariffs` rather than the single US years=10 data key.
- **`GET /api/trade/v1/get-trade-flows` request validation and coverage reporting** (#6309). Three consumer-facing changes. **(1)** `reporting_country` and `partner_country` must now be 3-digit UN M49 codes and `years` must be 0-30; a malformed value returns HTTP 400 instead of being silently replaced with `840` / `000` / `10`, which previously answered about a country the caller never asked for. Omitted parameters still take those documented defaults. **(2)** `years` is no longer restricted in practice to 10. The seed now holds a 30-year window per reporter and the endpoint slices it, so every lookback in 1-30 is served; requests for any other window previously returned `upstream_unavailable: true`. **(3)** The response gains `unavailable_reason` — the `TradeFlowUnavailableReason` enum (`NOT_COVERED`, `SEED_MISSING`, `COVERAGE_UNKNOWN`, `CACHE_UNAVAILABLE`, `INVALID_REQUEST`, `UNSPECIFIED` when rows are served) — plus `coverage_start_year` / `coverage_end_year`. `upstream_unavailable` is now `false` for `NOT_COVERED` — a reporter/partner combination outside seeded coverage is a contract answer, not an outage, and retrying will not help. Coverage is currently reporter-versus-World (`partner_country=000`) only: the WTO indicators behind these rows publish a World total and answer HTTP 204 for every named partner.

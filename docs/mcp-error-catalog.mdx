...
| `-32002`  | Terminal entitlement denial — a confirmed lapse, or a tool outside your tier | **403**             | Do NOT re-authenticate (the identity is still valid). Subscribe / resubscribe to restore access |
| `-32029`  | Rate-limited — per-minute throttle, Pro daily cap, OR free-account allowance | **200** (per-min) / **429** (daily) | Honour `Retry-After`; for 200/per-min back off ~1s                    |
...
**Not** in this list, deliberately: a caller whose subscription is inactive. Only a confirmed free account (a configured no-row result or an internally consistent tier-0 row) is admitted onto the free-account allowance (see `-32029` and `-32002` below). An expired or disabled paid entitlement returns terminal `-32002` at HTTP 403 because re-authenticating cannot fix it; provider-confirmed lapses use the more specific `lapsed-subscription` reason. An entitlement lookup that fails or cannot be verified returns a retryable `-32603` at HTTP 503 — it is an availability failure, not a verdict about the caller.

Every `-32001` carries a machine-readable `data` payload: `reason` is always `no-account`, `nextStep` is one sentence of guidance, and `upgradeUrl` is where to send the user. Branch on `error.data.reason`, not on the message text.
...
Always HTTP **403**, `Cache-Control: no-store`, and **no** `WWW-Authenticate` header — the credential is valid, the entitlement is not, and re-authenticating cannot change that. Two triggers, distinguished by `data.reason`:

**`reason: "lapsed-subscription"`** — a lapse the billing provider has confirmed. Fires from the entitlement pre-check (`api/mcp/auth.ts` `getMcpBillingVerificationDenial`) and, for a lapse landing mid-call between the pre-check and a tool's downstream fetch, from `api/mcp/dispatch.ts` (typed `BillingDenialError` re-emit). Also sets `X-Billing-Verification: subscription_lapsed`.
...
**`reason: "upgrade-required"`** — a signed-in account on the free allowance called a tool outside it. The free allowance covers **cached-data tools only**; tools that fetch live data downstream stay Pro-only. Fires from `api/mcp/dispatch.ts` before any allowance slot is charged, so a refused call costs the caller nothing. No `X-Billing-Verification` header (nothing is being verified).
...
All three triggers share this code; the HTTP status disambiguates the per-minute case, and `error.data.reason` disambiguates the free allowance from the Pro daily cap.
...
| **403** | JSON-RPC envelope         | `-32002`         | Terminal entitlement denial: `data.reason: "lapsed-subscription"` (confirmed lapse; also sets `X-Billing-Verification: subscription_lapsed`) or `"upgrade-required"` (free allowance called a Pro-only tool). No `WWW-Authenticate` (re-auth cannot fix it). |
